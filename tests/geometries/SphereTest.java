...
class SphereTest {
    ...
    void findIntersections() {
        try {
            ...
            List<GeoPoint> result = sphere.findIntersections(new Ray(new Point3D(1, 4, 1), new Vector(-1, -1, -1)));
...
//            assertThrows(ZeroVectorException.class, () -> sphere.findIntersections(new Ray(new Point3D(1, 2, 3), Vector.ZERO)));
            ...
            assertEquals(Intersectable.EmptyList, sphere.findIntersections(new Ray(new Point3D(4,0,0), new Vector(1,0,0))));
        } catch (Exception ex) {
            fail(ex.getMessage());
        }
    }
